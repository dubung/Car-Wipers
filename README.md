...
<p align="center">
  <video src="carwiper1.webm" controls width="640"></video>
</p>

[👉 `carwiper1.webm` 파일 다운로드](./carwiper1.webm)

### 🔹 추가 동작 데모

<p align="center">
  <video src="carwiper.webm" controls width="640"></video>
</p>

[👉 `carwiper.webm` 파일 다운로드](./carwiper.webm)
...
1. **CubeMX 설정 로드**
   - 제공된 `.ioc` 파일을 열거나, 위의 CubeMX 설정을 참고하여 핀/클럭/타이머/ADC/I2C/EXTI 설정
2. **코드 생성**
   - `GENERATE CODE` 버튼으로 프로젝트 생성
3. **사용자 코드 작성**
   - `main.c`에서
     - ADC 값을 읽어 조이스틱/센서 값 처리
     - TIM PWM으로 서보 모터 제어
     - I2C LCD에 현재 모드/수위 단계 표시
4. **빌드 및 다운로드**
   - STM32CubeIDE에서 빌드 후 보드에 플래시
5. **실행**
   - 전원 인가 후
     - 조이스틱으로 수동 모드 확인
     - 버튼으로 Auto 모드로 전환 후 물 수위 센서에 물을 가감하며 자동 동작 확인
   - 상단의 WebM 데모 영상과 같은 동작이 나오면 성공

---

## 9. 폴더 구조 예시

리포지토리는 예를 들어 아래와 같이 구성할 수 있습니다.

```text
.
├── Core/                  # STM32CubeIDE 생성 코드
├── Drivers/
├── README.md
├── carwiper1.webm
├── carwiper.webm
